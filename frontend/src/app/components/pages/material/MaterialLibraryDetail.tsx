    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Button type="button" onClick={handleSave} disabled={saveMutation.isPending} aria-busy={saveMutation.isPending}>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <label className="flex flex-col gap-1 text-sm text-foreground md:col-span-2">
            ...
            <textarea
              ...
              className="w-full rounded-md border border-border px-3 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-ring/40 aria-invalid:border-destructive/30"
            />
            {reasonMissing && <span className="text-sm text-destructive">{t("codeRuleDetail.changeReasonRequired")}</span>}
          </label>
          <label className="flex flex-col gap-1 text-sm text-foreground md:col-span-2">
        ...
        <div className="rounded-md border border-border bg-card p-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              {!preview.error && (
                <p className="mt-2 break-all font-mono text-lg text-primary">{preview.code || t("codeRule.emptyPreview")}</p>
              )}
            </div>
            ...
            <article
              ...
              className={`rounded-lg border bg-card p-4 ${
                showValidation && segmentValidationError(segment, t)
                  ? "border-destructive/30 ring-2 ring-red-100"
                  : "border-border"
              }`}
            >
              <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              {showValidation && segmentValidationError(segment, t) && (
                <p className="mt-3 text-sm text-destructive">{segmentValidationError(segment, t)}</p>
              )}
                ...
                {segment.type === "attribute_code" && (
                  <div className="flex flex-col gap-3 md:col-span-3">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <label className="inline-flex w-fit cursor-pointer items-center gap-2 rounded-md border border-primary/30 px-3 py-1.5 text-xs text-primary hover:bg-primary/10">
                  ...
                  <>
                    ...
                    {(segment.serialScope === "category" || segment.serialScope === "category_attribute") && (
                      <label className="flex flex-col gap-1 text-sm text-foreground">
                        ...
                        <select className="w-full rounded-md border border-border px-3 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-ring/40">
                    ...
                    <div className="rounded-md border border-primary/30 bg-primary/10 p-3 text-sm text-primary md:col-span-3">
                      <div className="font-medium">{t("codeRule.serialPreviewTitle")}</div>
                      <div className="mt-2 grid gap-2 md:grid-cols-3">
                        {serialScopePreviewRows(segment, t).map((row) => (
                          <div key={row.key} className="rounded border border-primary/30 bg-card px-3 py-2">
                            <div className="text-xs text-primary">{row.key}</div>
