    ...
    <>
      ...
      {type === "治理" && (
        <Button
          ...
          aria-busy={governanceImportMutation.isPending}
        >
          {governanceImportMutation.isPending ? "写入中..." : "确认批量写入"}
        </Button>
      )}
      {type === "添加" && (
        <Button type="button" onClick={() => addConfirmMutation.mutate()} disabled={!addPreview || addConfirmMutation.isPending} aria-busy={addConfirmMutation.isPending}>
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={modalTitle(type)} size="xl" footer={footer}>
      {type === "治理" && (
        <div className="space-y-5">
          <label className="block rounded-lg border-2 border-dashed border-border p-5 text-center text-sm text-muted-foreground hover:border-primary/30">
          ...
          <Button
            ...
            aria-busy={governancePreviewMutation.isPending}
          >
          ...
          {governanceImportMutation.isError && <p className="text-sm text-destructive">写入失败：{governanceImportMutation.error.message}</p>}
          {governanceSuccess && (
      ...
      {type === "添加" && (
        <div className="space-y-5">
          ...
          <Button type="button" variant="outline" onClick={analyzeDescription} disabled={!description.trim() || addPreviewMutation.isPending || !libraryId} aria-busy={addPreviewMutation.isPending}>
            <Sparkles className="h-4 w-4" />
          ...
          {addPreviewMutation.isError && <p className="text-sm text-destructive">预览失败：{addPreviewMutation.error.message}</p>}
          {addPreview && (
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {attributeEntries(addPreview).map(([key, value]) => (
                    <Badge key={key} variant="outline" className="border-primary/30 bg-primary/10 text-primary">
                      {key}: {value}
                    </Badge>
                  ))}
                </div>
          ...
          {addConfirmMutation.isError && <p className="text-sm text-destructive">创建失败：{addConfirmMutation.error.message}</p>}
          {addSuccess && (
            <div className="flex items-center gap-2 rounded-md border border-success/30 bg-success/10 p-3 text-sm text-success">
              <CheckCircle2 className="h-4 w-4" />
              {addSuccess}
      ...
      {type === "匹配" && (
        <div className="space-y-5">
          <label className="space-y-2 text-sm text-foreground">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="min-w-0 flex-1 rounded-md border border-border px-3 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-ring/40"
                placeholder="输入物料名称、品牌、规格或描述"
              />
              <Button type="button" onClick={() => matchMutation.mutate()} disabled={!matchQuery.trim() || matchMutation.isPending || !libraryId} aria-busy={matchMutation.isPending}>
          ...
          {matchMutation.isError && <p className="text-sm text-destructive">匹配失败：{matchMutation.error.message}</p>}
