...
export function RecodePreviewModal({
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={onClose} title={t("codeRuleRecode.previewTitle")} size="xl">
        <div className="flex flex-col gap-5">
          ...
          {(isGenerating || rowsQuery.isLoading || executeMutation.isPending) && (
            <div role="status" className="rounded-md border border-primary/30 bg-primary/10 px-3 py-2 text-sm text-primary">
          ...
          <div className="flex flex-wrap justify-end gap-2">
            ...
            {hasConflicts && (
              <label className="inline-flex items-center gap-2 rounded-md border border-destructive/30 px-3 py-2 text-sm text-destructive">
                <input
                  type="checkbox"
            ...
            <Button type="button" onClick={() => setConfirmOpen(true)} disabled={!canExecute || executeMutation.isPending} aria-busy={executeMutation.isPending}>
      ...
      <AlertDialog open={confirmOpen} onOpenChange={(open) => !executeMutation.isPending && setConfirmOpen(open)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("codeRuleRecode.executeConfirmTitle")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("codeRuleRecode.executeConfirmBody", { library: library.name, count: batch?.total_count ?? 0 })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={executeMutation.isPending}>{t("action.cancel")}</AlertDialogCancel>
            <AlertDialogAction
      ...
      <AlertDialog open={forceConfirmOpen} onOpenChange={(open) => !executeMutation.isPending && setForceConfirmOpen(open)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("codeRuleRecode.forceConfirmTitle")}</AlertDialogTitle>
            <AlertDialogDescription>{t("codeRuleRecode.forceConfirmBody")}</AlertDialogDescription>
          </AlertDialogHeader>
...
function RecodeBatchDetail({
  ...
  return (
    <div className="rounded-lg border border-primary/30 bg-muted/40 p-4">
      ...
      <AlertDialog open={rollbackOpen} onOpenChange={(open) => !rollbackMutation.isPending && setRollbackOpen(open)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("codeRuleRecode.rollbackConfirmTitle")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("codeRuleRecode.rollbackConfirmBody", { batch: batch.batch_id })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <p className="rounded-md border border-warning/30 bg-warning/10 px-3 py-2 text-sm text-warning">
            {t("codeRuleRecode.externalWarning")}
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={rollbackMutation.isPending}>{t("action.cancel")}</AlertDialogCancel>
            <AlertDialogAction
              className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
              disabled={rollbackMutation.isPending}
              aria-busy={rollbackMutation.isPending}
              onClick={(event) => {
                event.preventDefault();
          ...
          <tbody className="divide-y divide-border">
            {visibleRows.map((row) => (
              <tr key={row.id} className="hover:bg-muted/40">
                ...
                <td className="px-3 py-2 font-mono text-foreground">{row.new_code}</td>
                <td className="px-3 py-2 text-foreground">{row.material_name}</td>
                <td className="px-3 py-2 font-mono text-primary">{row.batch_id ?? "-"}</td>
