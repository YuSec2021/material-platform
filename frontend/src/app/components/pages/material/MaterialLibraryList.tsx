...
function AssociationTags({ names, emptyLabel }: { names: string[]; emptyLabel: string }) {
  ...
  return (
    <div className="mt-3 flex flex-wrap gap-1.5">
      {names.map((name) => (
        <Badge key={name} variant="outline" className="border-primary/30 bg-primary/10 text-primary">
          {name}
        </Badge>
      ))}
    </div>
    ...
    <div className="space-y-2 text-sm text-foreground">
      ...
      <div className="rounded-md border border-border bg-card p-3">
        ...
        <input
          ...
          placeholder={label}
          className="mt-2 w-full rounded-md border border-border px-3 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-ring/40"
        />
...
export function MaterialLibraryList() {
  ...
  const [libraryPendingDelete, setLibraryPendingDelete] = useState<MaterialLibrary | null>(null);
  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col gap-6 overflow-hidden">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {data.map((item) => (
              ...
              <div className="mb-4 flex items-start justify-between gap-3">
                ...
                <Badge
                  variant="outline"
                  className={item.enabled ? "border-success/30 bg-success/10 text-success" : "border-border bg-muted/40 text-muted-foreground"}
                >
                  {item.enabled ? t("status.enabled") : t("status.disabled")}
                </Badge>
              </div>
              <Badge variant="outline" className="mb-3 inline-flex items-center gap-1 border-primary/30 bg-primary/10 text-primary">
              ...
              <button
                ...
                className="mb-1 block text-left text-lg font-medium text-foreground hover:text-primary"
              >
                {item.name}
              </button>
              ...
              {item.auto_code_enabled && (
                <div className="mt-4 rounded-md border border-primary/30 bg-primary/10 px-3 py-2 text-xs text-primary">
              ...
              <div className="mt-5 flex gap-2">
                ...
                <Button
                  type="button"
                  onClick={() => setLibraryPendingDelete(item)}
      ...
      >
        <div className="space-y-5">
          ...
          {isCreateMode && form.autoCodeEnabled && (
            <section className="space-y-4 rounded-lg border border-primary/30 bg-muted/40 p-4">
              ...
              <div className="flex flex-wrap gap-2" aria-label={t("codeRule.addSegment")}>
                {segmentTypes.map((type) => (
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-md border border-primary/30 bg-card px-3 py-1.5 text-xs text-primary hover:bg-primary/10"
                  >
                          ...
                          <div className="space-y-3 md:col-span-3">
                            ...
                            <div className="space-y-2">
                              ...
                              {segment.mappings.map((row) => (
                                <div key={row.id} className="grid gap-2 md:grid-cols-[1fr_1fr_auto]">
                                  ...
                                  <button
                                    ...
                                    className="rounded-md border border-destructive/30 px-3 py-2 text-sm text-destructive hover:bg-destructive/10 disabled:cursor-not-allowed disabled:opacity-40"
                                  >
                                    {t("action.delete")}
                                  </button>
                                </div>
                              ...
                              <button
                                ...
                                className="inline-flex items-center gap-1 rounded-md border border-primary/30 px-3 py-1.5 text-xs text-primary hover:bg-primary/10"
                              >
                                <Plus className="h-3.5 w-3.5" />
      ...
      <AlertDialog
        open={Boolean(libraryPendingDelete)}
        onOpenChange={(open) => {
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>删除物料库</AlertDialogTitle>
            <AlertDialogDescription>
              确定删除物料库“{libraryPendingDelete?.name}”吗？该操作不可撤销。
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={(event) => {
                ...
                if (libraryPendingDelete) {
                  deleteMutation.mutate(libraryPendingDelete.id);
                }
              }}
            >
