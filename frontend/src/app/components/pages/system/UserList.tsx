...
export function UserList() {
  ...
  const [pendingAction, setPendingAction] = useState<{ kind: "reset" | "delete"; user: User } | null>(null);
  ...
  const confirmPendingAction = () => {
    ...
    setHcmMessage("");
    if (pendingAction.kind === "reset") resetMutation.mutate(pendingAction.user);
    else deleteMutation.mutate(pendingAction.user);
  ...
  return (
    <div className="space-y-6">
      ...
      <AlertDialog open={Boolean(pendingAction)} onOpenChange={(open) => !open && setPendingAction(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{pendingAction?.kind === "delete" ? "删除本地用户" : "重置用户密码"}</AlertDialogTitle>
            <AlertDialogDescription>
              {pendingAction?.kind === "delete"
                ? `确定删除用户 ${pendingAction.user.username} 吗？此操作无法撤销。`
                : `确定重置用户 ${pendingAction?.user.username ?? ""} 的密码吗？系统将生成新的临时密码。`}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              aria-busy={resetMutation.isPending || deleteMutation.isPending}
              className={pendingAction?.kind === "delete" ? "bg-destructive text-destructive-foreground hover:bg-destructive/90" : undefined}
            >
              {pendingAction?.kind === "delete" ? "确认删除" : "确认重置"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
