...
description: Maintain observability, unattended loop state, escalation summaries, and context-compaction artifacts for the Codex + Codex sprint harness. Use when the harness needs structured run-state updates, append-only event logs, routing audit logs, human handoff summaries, or pause/retry decisions for unattended execution.
---

# Harness Observability

Use this skill when working on the Codex + Codex sprint harness and you need to:
...
- add a brief pause note to `Codex-progress.txt`
...
- Do not turn `Codex-progress.txt` into an audit log
...
- `Codex-progress.txt`: compact rolling handoff only
